...
public class ListenerImpl implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
    	...
    	if (!yugioh.exists()) {
	        ...
	        HTreeMap<Integer, CartaYugioh> cartaMap = db.hashMap("yugioh.db")
	        	    .keySerializer(org.mapdb.Serializer.INTEGER)
	        	    .valueSerializer(org.mapdb.Serializer.JAVA)
	        	    .createOrOpen();
	        
	        for (int i = 1; i <= listaYugioh.size(); i++) {
	        	cartaMap.put(i, listaYugioh.get(i - 1));
	        	//System.out.println(listaYugioh.get(i - 1).name);
	        }
    	...
    	if (!magic.exists()) {
	        ...
	        HTreeMap<Integer, CartaMagic> cartaMap = db.hashMap("magic.db")
	        	    .keySerializer(org.mapdb.Serializer.INTEGER)
	        	    .valueSerializer(org.mapdb.Serializer.JAVA)
	        	    .createOrOpen();
	        
	        for (int i = 1; i <= listaMagic.size(); i++) {
	        	cartaMap.put(i, listaMagic.get(i - 1));
	        	//System.out.println(listaMagic.get(i - 1).name);
	        }
    ...
    private void caricaCartePokemon() {
    	...
    	if (!pokemon.exists()) {
	        ...
	        HTreeMap<Integer, CartaPokemon> cartaMap = db.hashMap("pokemon.db")
	        	    .keySerializer(org.mapdb.Serializer.INTEGER)
	        	    .valueSerializer(org.mapdb.Serializer.JAVA)
	        	    .createOrOpen();
	        
	        for (int i = 1; i <= listaPokemon.size(); i++) {
	        	cartaMap.put(i, listaPokemon.get(i - 1));
	        	System.out.println(listaPokemon.get(i - 1).name);
	        }
